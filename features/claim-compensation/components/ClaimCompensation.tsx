...
import ModalClaimCompensation from "./ModalClaimCompensation";

import useClaimCompensation from "@/stores/claim-compensation";
import { TSearchParams } from "@/types";
import { useSetParams } from "@/utils/set-params";
import { useClaimCompensationList } from "../hooks/useClaimCompensation";
import { hasPermission } from "@/utils/get-permission";

export default function MasterData() {
  // variables
  const { toggleModalClaimCompensation } = useClaimCompensation();
  ...
  const { data } = useClaimCompensationList(queryParams);
    ...
    <Card>
      <CardHeader className="flex flex-col gap-2 md:flex-row md:justify-between md:items-center md:gap-4 w-full">
        ...
        <div className="flex flex-col sm:flex-row w-full md:w-[60%] items-center justify-end gap-2">
          ...
          {hasPermission("Claim & Compensation", "create") && (
            <Button onClick={() => toggleModalClaimCompensation(true)}>
              <Plus size={16} />
              Add Payments
            </Button>
          )}
